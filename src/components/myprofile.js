...
card=(item,type) => {
    ...
    return(
        ...
        <Link to={{
          pathname:type==='mine'?'/mybookdetail':'/detail',
          state:item
...
showUserBooks=()=>{
    return this.state.userbooks.map((item,index)=>{
        return this.card(item,'mine');
    })
}

showUserLikes=() => {
    return this.state.userlikes.map((item,index)=>{
        return this.card(item,'like');
    })
}

delComment=async (commentId) => {
  try {
    await fetch(BASE_URL+'comments/'+commentId+'/parent',{
      method:'DELETE',
      headers: {
        'Accept': "application/json",
       'Content-Type': 'application/json',
...
editComment=(commentId) => {
  this.setState({
    operateCommentInput:commentId
  })
}
submitCommentChange= async (commentId) => {
  let commentChange=this.commentChange.value.trim();
  if(commentChange!==''){
  let data={
        ...
        return (
          <div className="card text-dark bg-light" key={item.id} style={{maxHeight:'200px',overflow:'auto'}} onMouseEnter={()=>{this.setState({operateCommentId:item.id})}} onMouseLeave={()=>{this.setState({operateCommentId:''})}}>
            <div className="card-header">
                ...
                <span className="float-end" style={{fontStyle:'italic',marginLeft:'20px',fontSize:'0.8rem'}}>- - By: {item.book.author}</span> 
                </div>
               <div className="card-body">
               {this.state.operateCommentInput===item.id?<textarea type="text" style={{width:'95%',padding:'0px'}} defaultValue={item.content} ref={input=>{this.commentChange=input}}/>:
               <p className="card-text lh-sm" style={{fontSize:'0.9rem'}}>{item.content}</p>}
               {this.state.operateCommentId===item.id && this.state.operateCommentInput!==item.id?<div >
               <button className="btn btn-sm btn-outline-danger float-end ms-1" onClick={()=>{this.delComment(item.id)}}>Delete</button>
               <button className="btn btn-sm btn-outline-success float-end" onClick={()=>{this.editComment(item.id)}}>Edit</button>               
               </div>:''}
               {this.state.operateCommentInput===item.id && <button className="btn btn-sm btn-outline-primary ms-2" onClick={()=>{this.submitCommentChange(item.id)}}>OK</button>}
               </div>
            </div>
      );
    })
        ...
        <Fragment>
            <div className="card userDetails">
                <div className="row g-0">
                    <div className="col-md-1 col-12 text-center userProfile p-2">
                    ...
                    <div className="col-md-11 col-12 userInfo">
                        <div className="card-body p-0 ps-3">
                            <div className="userBooks">
                                <h5 className="card-title m-2 ms-0 me-0 p-2 bg-info">My Uploads :</h5>
                                <div className="homeContainer" style={{ width: "100%", height: "auto", padding: "0px"}}>
                                  <div className="row row-cols-lg-6 row-cols-md-4 row-cols-sm-2 row-cols-1">
                                   {this.showUserBooks()}
                                  </div>
                                </div>
                            </div>
                          ...
                          <ul className="pagination justify-content-center m-2">
                            ...
                            <div className="userLikes mt-3">
                                <h5 className="card-title m-2 ms-0 me-0 p-2 bg-warning">My Likes :</h5>
                                <div className="homeContainer" style={{ width: "100%", height: "auto", padding: "0px"}}>
                                  <div className="row row-cols-lg-6 row-cols-md-4 row-cols-sm-2 row-cols-1">
                                {this.showUserLikes()}
                                </div>
                            ...
                            <div className="userComments mt-3">
                                <h5 className="card-title m-2 ms-0 me-0 p-2" style={{backgroundColor:'lightpink'}}>My Comments :</h5>
...
export default connect(mapState,null)(UserDetail)